...
export default function NavbarSection() {
  function linkhidden(href:string,title:string){
    if(session.status=="unauthenticated"&&(href=="/cart"||href=="/allorders"||href=="/whishlist")){
      return null
    }
     
      return (<div>
        <Link className=' relative mr-2' href={href} >{ title}

                  {href=="/cart"&& <Badge variant="secondary" className='mx-0 absolute top-[-10] right-[-1px] bg-black w-[20px] h-[20px] rounded-full text-white'>{cartNumber>0?cartNumber:0}</Badge>
}
              </Link>
      </div>
 ...
 if(token){
  ...
  const router=useRouter()
     const {cartNumber}=useContext(cartProductCount)
       return (
    <nav className="  flex  items-center justify-between   py-[20px]  sticky top-0 z-30 nav-Back-ground shadow-nav">
  ...
  <div className="hidden gap-3 lg:inline-flex">
        ...
        {navItems.map((item) => (
          ...
          <Button key={item.title} asChild variant={'ghost'}>
            {linkhidden(item.href,item.title)}
      ...
      <div className="   lg-slex">
        {
          session.status=="authenticated"?  <Button className='bg-green-700 rounded-2xl p-3 text-white me-6' onClick={function(){signOut({callbackUrl:"/api/auth/signin"})}}>
          Sign Out
        </Button>:<div>


        <Button className='bg-green-700 rounded-2xl p-3 text-white me-6' onClick={function(){router.push("/auth/signin")}}>
          Sign in
        </Button>


        <Button className='bg-green-700 rounded-2xl p-3 text-white me-6' onClick={function(){router.push("/signup")}}>
          Sign up
        </Button>
        </div> 
